...
ccn_charbuf_putf(struct ccn_charbuf *c, const char *fmt, ...)
{
    ...
    va_list ap;
    char *buf;
    buf = (char *)ccn_charbuf_reserve(c, strlen(fmt) + 10); /* estimate */ //TODO CONET when used by ccncatchunk(or other apps) to put the new chumk number, brings to inconsistency with conet capabilities on max chunk  number support(6-bytes and part of 5-bytes CSN)
