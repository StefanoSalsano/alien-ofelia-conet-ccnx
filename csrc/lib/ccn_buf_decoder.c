...
int
ccn_parse_ContentObject(const unsigned char *msg, size_t size, struct ccn_parsed_ContentObject *x, struct ccn_indexbuf *components)
{
//    fprintf(stderr,"\n[ccn_buf_decoder:%d]Sono dentro ccn_buf_decoder\n",__LINE__);
    struct ccn_buf_decoder decoder;
    struct ccn_buf_decoder *d = ccn_buf_decoder_start(&decoder, msg, size);
    int res;
    x->magic = 20090415;
...
//        fprintf(stderr,"\n[ccn_buf_decoder:%d]Sono dentro il primo if e res=%d\n",__LINE__,res);
        if (res < 0)
            d->decoder.state = -__LINE__;
        x->name_ncomps = res;
		...
		d->decoder.state = -__LINE__;
//		fprintf(stderr, "\n[ccn_buf_decoder:%d]Sono nell else\n", __LINE__);
	}
	if (d->decoder.index != size || !CCN_FINAL_DSTATE(d->decoder.state)) {
		//fprintf(stderr,"\n[ccn_buf_decoder:%d]Sono nell ultimo if e sto ritornando %d\n",__LINE__, CCN_DSTATE_ERR_CODING);
		return(CCN_DSTATE_ERR_CODING);
	}
	return(0);
}
