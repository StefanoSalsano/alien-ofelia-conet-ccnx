...
struct face *record_connection(struct ccnd_handle *h, 	int fd,
                                      struct sockaddr *who,
                                      socklen_t wholen,
                                      int setflags);
...
consume_interest(struct ccnd_handle *h, struct interest_entry *ie)
{
    ...
    if (res != HT_OLD_ENTRY){
        	//abort(); whr
		printf(stderr,"!!!!!!!!!!!!!! consume_interest\n");
                  ...
                  int setflags)
{
    ...
    res = fcntl(fd, F_SETFL, O_NONBLOCK); //TODO maybe this setting also in fds of CONET
    if (res == -1)
        ccnd_msg(h, "fcntl: %s", strerror(errno));
    hashtb_start(h->faces_by_fd, e);
    if (hashtb_seek(e, &fd, sizeof(fd), wholen) == HT_NEW_ENTRY) {
		//in_port_t port_n = ntohs(((struct sockaddr_in*)who)->sin_port);//[CONET]: only for printing ccnd an conet. No conet setting
		//printf("\nCONET: recording port %d\n", port_n);
        face = e->data;
        face->recv_fd = fd;
                   ...
                   struct nameprefix_entry *npe)
{
    ...
    if (ie->ev != NULL && wt_compare(expiry + 2, ie->ev->evint) < 0)
        ccn_schedule_cancel(h->sched, ie->ev);
    if (ie->ev == NULL){
		//XXX conet modification
		if (USE_CONET && check_for_internal_ccn_interest(ie->interest_msg, ie->size)!=0){

			struct  conet_entry* ce=NULL;

			int chunk_number=0;
			struct ccn_charbuf *name=ccn_charbuf_create();
			int csn_size = ccnb_extract_info(pi, ie->interest_msg, ie->size, &chunk_number, name);

			char*nid = (char*)name->buf;
#ifndef IS_CLIENT
			if(ce_cache!=NULL && memcmp(ce_cache->nid, nid, name->length)==0){
				ce=ce_cache;
			}
#else
			//metodo (brutto) per evitare di dover riavviare il client ogni volta
			//per ogni interest controlliamo se il chunk number e' zero,
			//se e' 0 elimino la nid dalla hashtable in modo da avere una conet_entry nuova
			if (ce_cache != NULL && memcmp(ce_cache->nid, nid, name->length)== 0 && chunk_number != 0) {
				ce = ce_cache;
			}
			if (chunk_number == 0)
			{
				struct hashtb_enumerator ee2;
				struct hashtb_enumerator *e2 = &ee2;
				hashtb_start(h->conetht, e2);
				hashtb_seek(e2, nid, strlen(nid), 0);
				hashtb_delete(e2);
				hashtb_end(e2);
			}
#endif
			//int chunk_number = chu_num;

			if(CONET_DEBUG >= 2) fprintf(stderr, "[ccnd.c:%d ]Interest from ccnx for %s/%d \n", __LINE__, nid, chunk_number);
			if (ce==NULL){
				struct hashtb_enumerator ee;
				struct hashtb_enumerator *e = &ee;
				...
				if (res < 0){
					...
					exit(-1); //hash table is broken here, nothing to do
				}
				...
				if (res == HT_NEW_ENTRY) { //new entry in the hash table of the nids
					...
					ce->in_flight=0;
					ce->in_fast_rec=0;
					ce->threshold=SS_THRESH; //it's about 63535/1500
					ce->wnd_adder=0;
					ce->chunk_size=0;
					ce->nid=nid;
			...
			int resc = hashtb_seek(e1,&chunk_number, sizeof(int),0);
			if(resc == -1){
				if(CONET_DEBUG >= 2) fprintf(stderr, "[ccnd.c:%d]: error in seeking or creating chunks in hash table\n", __LINE__);
				hashtb_end(e1);
			...
			if (resc==HT_NEW_ENTRY){  //new entry in the hash table of chunks for a nid
				...
				ch->m_segment_size = get_segment_size(nid,chunk_number, ce->chunk_size, CONET_DEFAULT_MTU);
				ch->starting_segment_size = ch->m_segment_size;
				ch->bytes_not_sent_on_starting = 0;
				ch->last_interest=0;
				ch->last_in_sequence=0;
				ch->last_possible_interest=0; //SS it was 1 I initialized it to 0 because it is automatically increased
				ch->final_segment_received=0;
				ch->tries=0;
				ch->send_started=0;
				if (ce->chunk_size==0){
					/**
					 * this  the first  chunk requested for this nid, we allocate only one bit for the first
					 * carrier packet to be sent, the first carrier data packet will carry chunk size information
					 */
					ch->sending_interest_table= calloc(1, sizeof(conet_bit));
					ch->expected= calloc(1, sizeof(conet_bit));
					ch->expected[0].bit = 1;
					ch->out_of_sequence_received=calloc(1, sizeof(conet_bit));
					ch->bitmap_size =1;
					if(CONET_DEBUG >= 2) fprintf(stderr, "[ccnd.c:%d]: NEW chunk & chunk_size==0\n", __LINE__);
				}
				else {
					if(CONET_DEBUG >= 2) fprintf(stderr, "[ccnd.c:%d]: NEW chunk (not the first one for a nid) : setup_chunk_bitmaps\n", __LINE__);
					setup_chunk_bitmaps(ce, ch);
				}
				ch->return_faceid = face->faceid;
				ch->send_faceid=(outbound==NULL || outbound->n==0)?-1:outbound->buf[0];
				ch->current_size = 0;
				ch->timeout = calloc(1, sizeof(struct timeval));
				ch->timeout->tv_sec = 0;
				ch->timeout->tv_usec = 0;
				ch->last_sent = calloc(1, sizeof(struct timeval));
				ch->last_sent->tv_sec = 0;
				ch->last_sent->tv_usec = 0;
				ch->reschedule_send_ev = NULL;
				ch->never_arrived=0;
				ch->arrived=0;
				ch->outbound = get_outbound_faces(h, face, msg, pi, npe);
				ch->is_oos=0;
				ch->to=NULL;
				/*ch->outbound = malloc(sizeof(pe->outbound));
				memcpy(ch->outbound, pe->outbound, sizeof(pe->outbound));
				 */

				//ce->last_processed_chunk=ch->chunk_number;

			}
			else if (resc==HT_OLD_ENTRY) { //the chunk was already in the hash table of chunks for a nid

				if(CONET_DEBUG >= 2) fprintf(stderr, "[ccnd.c:%d ]The interest for %s/%d is already managed by conet, dropping the ccnx interest. Tries: %d\n", __LINE__, nid, ch->chunk_number, ch->tries);
				//i only refresh the outbound for this interest
				if (ch->outbound->buf==NULL){

					ch->outbound=get_outbound_faces(h, face, msg, pi, npe);

				}

//					if (ch->never_arrived==1 ){
//						ch->never_arrived=0;
//						fill_the_cwnd(h, ce, ch);
//					}

				if (ce->cwnd>ce->in_flight){
					//printf(stderr, "[ccnd.c:%d ]Recover an open cwnd lcp:%d c_exp:%d\n", __LINE__, ce->last_processed_chunk, ce->chunk_expected->chunk_number);
					if (ce->chunk_expected==NULL && ce->last_processed_chunk>ch->chunk_number){
						ce->chunk_expected=ch;
					}
					if (ce->chunk_expected!=NULL && ce->chunk_expected->chunk_number==ch->chunk_number){
						if(CONET_DEBUG >= 2) fprintf(stderr, "[ccnd.c:%d ]Resched send is_final:%d lis %d li %d\n", __LINE__, ch->final_segment_received, ch->last_in_sequence, ch->last_interest);
						ch->is_oos=1;
						conet_send_interest_cp(h, ce, ch, 1);
					}
					else if (ce->last_processed_chunk>ch->chunk_number){
						ch->is_oos=1;
						if(CONET_DEBUG >= 2) fprintf(stderr, "[ccnd.c:%d ]Resched send not exp\n", __LINE__);
						conet_send_interest_cp(h, ce, ch, 1);
					}
					else {
						if(CONET_DEBUG >= 2) fprintf(stderr, "[ccnd.c:%d ]fill the cwnd\n", __LINE__);
						if(CONET_DEBUG >= 2)fprintf(stderr, "\n last_in_seq: %d lcp: %d \n ", ch->last_in_sequence, ce->last_processed_chunk);

						fill_the_cwnd(h, ce, ch);
					}
				}
				//TODO maybe a forced retransmit procedure must be set
				hashtb_end(e1);
				//return 0;
				res = 0;
				goto Bail;
			...
			if (ce->cwnd > ce->in_flight  ){
				...
				if (ce->chunk_size!=0){
					ch->last_possible_interest=ch->last_interest + (ce->cwnd - ce->in_flight);
				}
				if(CONET_DEBUG >= 1){
					fprintf (stderr, "[ccnd.c:");
					conet_print_stats(ce, ch, __LINE__);
				}

				ch->reschedule_send_ev=NULL;
				if (ce->chunk_expected==NULL){
					fill_the_cwnd(h, ce, ch); //TODO: if there are transmission problems in fill_the_cwnd
					                          //TODO: the inflight will be < than cwnd
					                          //TODO: but this means that in the next fill_the_cwnd around 3551
					                          //TODO: the lpi will be increased
					//conet_send_interest_cp(h, ce, ch, 0);
					ce->last_processed_chunk=ch->chunk_number;
				}
				else {
				}
			...
			ccnx_way:
			if(CONET_DEBUG >= 2) fprintf(stderr, "[ccnd.c:%d ]Propagate interest in the ccnx way\n", __LINE__);

			/*if (CONET_LOG_TO_FILE && check_for_internal_ccn_interest(msg, msg_out_size)!=0){
				struct ccn_charbuf *c = ccn_charbuf_create();
				ccn_uri_append(c, msg, msg_out_size, 0);
				char *uri = ccn_charbuf_as_string(c);
				char* last_component = strrchr(uri, '/')+1;
				int chunk_number = atoi(last_component);
				if (CONET_LOG_TO_FILE){
					if (out_file_ccn_si==NULL){
						out_file_ccn_si=fopen("out_ccn_si.csv", "wt");
					}
					log_to_file(out_file_ccn_si, chunk_number, 2, 3);
				}
				ccn_charbuf_destroy(&c);
			}*/
                         ...
                         unsigned char *wire_msg, size_t wire_size)
{
    ...
    if (res < 0) {
		...
		conet_print_ccnb(msg, size); //CONET
        goto Bail;
    }
	//[CONET] TODO XXX removed for test    ccnd_meter_bump(h, face->meter[FM_DATI], 1);
    if (comps->n < 1 ||
...
Bail:
    ...
    if (res >= 0 && content != NULL) {
        ...
        if (res == HT_NEW_ENTRY) {
			...
			if (n_matches == 0 && face!=NULL && (face->flags & CCN_FACE_GG) == 0) { //check if face not null (CONET)
        ...
        // in CONET va tolto q->send_queue->buf[i] quindi questo pezzo di codice e' inutile
        // forse e' inutile anche in ccnx
		//ripristinare se si trova il modo di non farla andare in crash quando si usano i carrier-packet (vecchio commento)
//		if(face != NULL){//if by [CONET]
//        for (c = 0; c < CCN_CQ_N; c++) {
//            q = face->q[c];
//            if (q != NULL) {
                      ...
                      unsigned char *msg, size_t size, int pdu_ok)
{
    ...
    switch (dtag) {
		...
		//if (CONET_DEBUG >= 2) fprintf(stderr,"BBB-1:%d", size);
		//get_uri_from_ccn(msg, size );
...
process_input(struct ccnd_handle *h, int fd)
{
    ...
    if (face->inbuf->length == 0)
        memset(d, 0, sizeof(*d));
	//CONET
	buf = ccn_charbuf_reserve(face->inbuf, 100000);
	if (buf==NULL) {
		ccnd_msg(h, "recvfrom face %u ********************************then error allocating buffer!", face->faceid);
	}
	else {

		memset(&sstor, 0, sizeof(sstor));
		res = recvfrom(face->recv_fd, buf, face->inbuf->limit
				- face->inbuf->length,
		...
		if (res == -1)
			...
			source = get_dgram_source(h, face, addr, addrlen, (res == 1) ? 1
					: 2);
			...
			msgstart = 0;
			if (((face->flags & CCN_FACE_UNDECIDED) != 0 && face->inbuf->length
					>= 6 && 0 == memcmp(face->inbuf->buf, "GET ", 4))) {
			...
			while (d->state == 0) {
				process_input_message(h, source, face->inbuf->buf + msgstart,
						d->index - msgstart, (face->flags & CCN_FACE_LOCAL)
								!= 0);
				msgstart = d->index;
				...
				dres = ccn_skeleton_decode(d, face->inbuf->buf + d->index, // XXX - msgstart and d->index are the same here - use msgstart
    ...
    for (h->running = 1; h->running;) {
        ...
        for (i = 0; res > 0 && i < h->nfds; i++) {
			if (h->fds[i].revents != 0) {
				res--;
				if (h->fds[i].fd==h->conet_fd || h->fds[i].fd == h->conet_raw_fd || h->fds[i].fd == h->conet_fd6){
					if (h->fds[i].revents & (POLLERR | POLLNVAL | POLLHUP)) {
						if (h->fds[i].revents & (POLLIN))
							if (CONET_DEBUG >= 2) fprintf(stderr, "\nCONET : SET HANDLING THIS RESPONSE!!! (1) \n");
						//process_input(h, h->conet_fds[i].fd); //TODO change function!!!
						else
							if (CONET_DEBUG >= 2) fprintf(stderr, "\nCONET: SET HANDLING THIS RESPONSE!!! (2) \n");

						//shutdown_client_fd(h, h->conet_fds[i].fd);
						continue;
					}
					if (h->fds[i].revents & (POLLIN)){
						double t0 = get_time();
						int bytes_read=conet_process_input(h, h->fds[i].fd);
						//if (CONET_DEBUG >= 2) //fprintf(stderr,"[ccnd.c: %d] bytes_read=%d with conet_process_input(..)\n",__LINE__,bytes_read);
						double t1 = get_time();
						//if (CONET_DEBUG >= 2) fprintf(stderr, "\nMU:%f\n", t1-t0);
						service_time+=t1-t0;
						//if (CONET_DEBUG >= 2) fprintf(stderr, "\nST: %f\n", service_time);
        ...
        if (res == 0) {
            for (a = addrinfo; a != NULL; a = a->ai_next) {
                ...
                if (fd != -1) {
					...
					//CONET increase your buffer
					int buffsize=1024*1024*10;
					if (setsockopt(fd, SOL_SOCKET, SO_RCVBUF, (char*)&buffsize, sizeof(buffsize))){
...
ccnd_create(const char *progname, ccnd_logger logger, void *loggerdata)
{
	...
	const char *listen_also_on = NULL; //[CONET]
    int fd;
	...
	h->face_limit = 1024-1; /* soft limit */  //[CONET] soft limit minus one( in CONET with only a socket listening on carrier packets)
    h->faces_by_faceid = calloc(h->face_limit, sizeof(h->faces_by_faceid[0]));
	//RESTORE when implementing carrier packet from multiple face/ h->conet_faces_by_fd = calloc(h->face_limit, sizeof(h->conet_faces_by_faceid[0]));//[CONET]
    param.finalize = &finalize_face;
    h->faces_by_fd = hashtb_create(sizeof(struct face), &param);
	//RESTORE when implementing carrier packet from multiple face/ h->conet_faces_by_fd = hashtb_create(sizeof(struct face), &param);//[CONET]
    ...
    h->portstr = portstr;
	portcp = CONET_DEFAULT_UNICAST_PORT; //[CONET]
	h->portcp = portcp; //[CONET]
    entrylimit = getenv("CCND_CAP");
	ccnd_msg(h, "getenc CCND_CAP: %s", entrylimit);
    h->capacity = ~0;
	if (entrylimit != NULL && entrylimit[0] != 0) {
        h->capacity = atol(entrylimit);
		if (h->capacity == 0) {
            h->force_zero_freshness = 1;
			ccnd_msg(h, "SETTING FORCE ZERO FRESHNESS");
		}
        if (h->capacity <= 0)
            h->capacity = 10;
    }
	...
	if(conet_listen_on(h, listen_also_on) < 0){
		if (CONET_DEBUG >= 2) fprintf(stderr, "\nError setting up CONET listening face\n");
		abort();
	}
	h->conetht = hashtb_create(sizeof(struct conet_entry), NULL);
#ifdef WHR
	h->s_pit = hashtb_create(sizeof(struct s_pit_entry), NULL);
#endif
	//	h->conet_content_cache =  hashtb_create(sizeof(struct conet_content), NULL);
	if(h->conetht == NULL){
		if (CONET_DEBUG >= 2) fprintf(stderr,"[CONET]: Error creating hashtable for CoNet\n"); //TODO handle this case
		abort();
	}
...
{
	process_incoming_content(h, face, wire_msg, wire_size);
}
