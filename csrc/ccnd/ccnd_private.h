...
struct ccnd_handle {
    ...
    //RESTORE when implementing carrier packet from multiple face/ struct hashtb *conet_faces_by_fd;/**<[CONET] CONET faces by fd*/
    ...
    //RESTORE when implementing carrier packet from multiple face/ struct face **conet_faces_by_faceid;/**< array with face_limit elements (CONET)*/
    ...
    const char *portcp; 			/**< [CONET] UDP port for carrier packet*/
    ...
    nfds_t conet_nfds;				/**< number of entries in conet fds array */ //[CONET]
    struct pollfd *fds;             /**< used for poll system call */
    struct pollfd *conet_fds;		/**< [CONET] used for poll system call (on CONET)*/
    int conet_fd;  					/**< [CONET] fd of receiving socket (on (only)one CONET socket)//TODO maybe redundant*/
    int conet_raw_fd;				/**< [CONET] fd of receiving RAW socket */

    int conet_fd6;					/**&iv: [CONET] valori extra per protocolli coesistenti */
    int conet_raw_fd6;
    struct pollfd *conet_fds6;
    nfds_t conet_nfds6;
    //RESTORE when implementing carrier packet from multiple face/ struct face *conet_face;
    struct hashtb * conetht;
    struct hashtb * conet_content_cache;
