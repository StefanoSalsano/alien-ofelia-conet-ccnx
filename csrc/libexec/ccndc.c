                        ...
                        int freshness)
{
    ...
    //&iv: modifiche necessarie ipv6 provvisoria
    //struct addrinfo hints = {.ai_family = AF_UNSPEC, .ai_flags = (AI_ADDRCONFIG)};
    struct addrinfo hints = {.ai_family = AF_UNSPEC, .ai_flags = 0}; //whr
//    struct addrinfo mcasthints = {.ai_family = AF_UNSPEC, .ai_flags = (AI_ADDRCONFIG | AI_NUMERICHOST)};
    struct addrinfo mcasthints = {.ai_family = AF_UNSPEC, .ai_flags = 0};// setssa cosa di sopore ma non e' testato whr
