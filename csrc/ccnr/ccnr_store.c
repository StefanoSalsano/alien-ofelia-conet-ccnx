...
//#ifdef K_MODULE devo importare conet.h
	 //porcata la dimensione serve per eliminare le chiavi alla creazione del repository
//	if ((content->cob != NULL && content->cob->length == content->size)&&content->flatname->length<80){
//		const unsigned char *ans = NULL;
//		ans = content->cob->buf;
//		fprintf(stderr, "........ %d\n", content->size);
//		    char file_name[255];
//			sprintf(file_name, "%s_%d", "/home/stefano/ccnrep/mod/examplemb1", chunk_counter);
//			FILE *fd = fopen(file_name, "w");
//			chunk_counter++;
//			//se il chunk e' piu piccolo sono alla fine del contenuto e azzero il contatore
//			if(content->size<4000)
//				chunk_counter = 0;
//			fwrite(ans, sizeof(char), content->cob->length, fd);
//			fclose(fd);
//	}
//#endif
