...
#define RTTE_TIMER_TICK 500000 // timer tick of RTTE in us
    ...
    if (incoming) {
        ...
        if (delta <= 30000000) {
            ...
            if (rtte < 127)
                rtte = delta;
            // Andrea Detti
            //rtte = ((rtte + RTTE_TIMER_TICK - 1) / RTTE_TIMER_TICK) * RTTE_TIMER_TICK;
            //rtte = RTTE_TIMER_TICK;
            // end Andrea Detti
            md->rtte = rtte;
        }
...
GOT_HERE();
        ...
        written = fwrite(data, data_size, 1, output_file);
        if (written != 1)
            exit(1);
        /* Check for EOF */
        ...
        while (md->ooo_count > 0 && md->ooo[slot].raw_data_size != 0) {
            ...
            written = fwrite(ooo->raw_data, ooo->raw_data_size - 1, 1, output_file);
            if (written != 1)
                exit(1);
            /* Check for EOF */
            if (slot == md->finalslot) {
